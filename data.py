...
class data_processor(arcs_data_paths, test_data_paths, CRay_data_path, numpix_side, max_noise_rms, max_psf_rms, max_cr_intensity, constant_noise_rms, variable_noise_rms, pix_res, min_unmasked_flux, num_data_dirs, max_xy_range, num_out):
    ...
    def get_data(self):
        ...
        self.constant_noise_rms = constant_noise_rms
    ...
    def read_batch_online(self, X , Y , max_file_num , train_or_test):
        ...
        selfYmat = Ymat.reshape((self.num_out,num_samp)).transpose()
        X[:] = Xmat
        Y[:] = Ymat
    ...
    def pick_new_lens_center(self, ARCS,Y, xy_range = 0.5):
        rand_state = np.random.get_state()
            while True:
                x_new = np.random.randint( -1 * np.ceil(self.xy_range/2/self.pix_res) , high = np.ceil(self.xy_range/2/self.pix_res) )
                y_new = np.random.randint( -1 * np.ceil(self.xy_range/2/self.pix_res) , high = np.ceil(self.xy_range/2/self.pix_res) )
                m_shift = - int(np.floor(Y[3]/self.pix_res) - x_new)
                n_shift = - int(np.floor(Y[4]/self.pix_res) - y_new)
                shifted_ARCS = im_shift(ARCS.reshape((self.numpix_side,self.numpix_side)), m_shift , n_shift ).reshape((self.numpix_side*self.numpix_side,))
    ...
    def read_data_batch(self, X , Y , mag , max_file_num , train_or_test):
        ...
        inds = np.random.randint(0, high = self.max_file_num , size= batch_size)
        ...
        for i in range(batch_size):
            ...
            while True:
                ...
                nt = 0
                    ...
                    ARCS_SHIFTED, lensXY , m_shift, n_shift = pick_new_lens_center(ARCS,Y[i,:], xy_range = self.max_xy_range)
                    LENS_SHIFTED = im_shift(LENS.reshape((self.numpix_side,self.numpix_side)), m_shift , n_shift ).reshape((self.numpix_side*self.numpix_side,))
            ...
            apply_psf(im_telescope , self.max_psf_rms , apply_prob = 0.8)
            add_poisson_noise(im_telescope , apply_prob = 0.8)
            add_cosmic_ray(im_telescope,apply_prob = 0.8 )
            add_gaussian_noise(im_telescope)
            mask = gen_masks( 30 , ARCS.reshape((self.numpix_side,self.numpix_side)) , apply_prob = 0.5 )
            mask = 1.0
            ...
            if np.any(np.isnan(X[i,:])) or np.any(np.isnan(Y[i,:])):
                ...
                selfY[i,:] = np.zeros((1,self.num_out))
